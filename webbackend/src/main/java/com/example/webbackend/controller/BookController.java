...
public class BookController {
    ...
    // PUT endpoint, used to update book
    @PutMapping("/books/{id}")
    ...
    // PATCH endpoint, used to partial update book
    @PatchMapping("/books/{id}")
    public Book partialUpdateBooks(@PathVariable Long id, @RequestBody Book partial_update
    ) {
        for (Book book : books) {
            if (book.getId().equals(id)) {
                if (partial_update.getTitle() != null) {
                    book.setTitle(partial_update.getTitle());
                }
                if (partial_update.getAuthor() != null) {
                    book.setAuthor(partial_update.getAuthor());
                }
                if (partial_update.getPrice() != null) {
                    book.setPrice(partial_update.getPrice());
                }
                return book;
            }
        }
    ...
    // GET endpoint, retrieve book depending on pagination
    @GetMapping("/books/pagnation")
    public List<Book> getPage(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "3") int page_size
    ) {
        return books.stream()
    ...
    // Advanced GET endpoint: Filtering, Sorting, Pagination combined in the valid order
    // Reference the functions above
    @GetMapping("/books/advanced")
    public List<Book> bookAdvanced(
            @RequestParam(required = false, defaultValue = "") String title,
